...
1. **Review** — run `code-reviewer.agent.md` and `test-adequacy-reviewer.agent.md` by reference, one after the other (the latter also gets `verification.test_commands`). If the reviewer returns `BLOCKED` or the adequacy verdict is `INEFFECTIVE`, gate (numbered list; user replies with the number):
   - **question:** "Review found blocking issues: <one line per finding>. How should I proceed?"
   - **header:** "Findings"
   - **options:**
     - **Fix now (Recommended)** — return to step 4 for the findings (step 3 if the plan itself is wrong), re-run step 5, then re-run both agents.
...
2. **Simplify** — once the review is clean, run `code-simplifier.agent.md` by reference. If it returns proposals (not `Clean`), gate (numbered list; user replies with the number):
...
- The step 3 challenge and step 5.5 reviews are read-only agent passes. The main session applies any accepted simplifier diff and re-verifies; blocking findings (`BLOCKED` / `INEFFECTIVE`) reach step 6 only via an explicit user waiver.
