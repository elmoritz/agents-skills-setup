...
- `review.agents` → the checkers the implementation loop runs each round (default `[code-reviewer, test-adequacy-reviewer]` when absent; a listed agent whose file is missing is skipped with a warning).
...
Once the evaluation says **Done**, run `code-simplifier.agent.md` by reference. If it returns proposals (not `Clean`), gate (numbered list; user replies with the number):
