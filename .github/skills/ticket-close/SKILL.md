...
> The gates below are presented as a numbered list: each option is written as `N. **Label** — description`, and the user replies with the number. Never silently pick an option that changes scope. Free-text follow-ups remain plain inline asks.
...
- If 2+ entries: present them as a numbered list the user picks by number:
  - **prompt:** "Which ticket should I close?"
...
Surface the ticket title and ask as a numbered list (user replies with the number):

- **prompt:** "Close `<id> — <title>` as shipped?"
- **options:**
  1. **Ship it** — proceed with closure (Recommended if the user invoked the command with an explicit ID).
  2. **Cancel** — leave the ticket in the close-source stage untouched. Stop.
