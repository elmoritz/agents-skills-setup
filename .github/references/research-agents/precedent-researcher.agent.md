...
2. Sweep past tickets: read the ticket root from `.github/config.yaml` (`backend.filesystem.root`, all stage folders including done) or, on the GitHub backend, search closed issues (`gh issue list --state all --search`). Past `## Decisions & assumptions` sections are the richest source.
