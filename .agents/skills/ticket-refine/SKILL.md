...
- Resolve the `inbox` role. **If null, the command is unavailable** — print `"This project has no inbox stage configured. /ticket-refine is unavailable. Edit .agents/config.yaml to add a stage with `roles: [inbox]` if you want to enable refinement."` and stop.
