---
name: ticket-engine
description: The shared execution layer behind every /ticket-* skill — loads and validates .agents/config.yaml, assigns IDs, runs backend stage transitions, formats commit/comment messages, and reports half-state on partial failure. Used by the ticket skills and milestone-sync, not invoked directly.
user-invocable: false
---
<!-- sync:divergent -->
# ticket-engine

*Project-agnostic execution layer for the /ticket-* commands. Loads .agents/config.yaml, validates the schema, resolves roles to stages, assigns IDs, runs stage transitions on the configured backend (filesystem or GitHub), formats commit/comment messages, and reports half-state on partial failure. Invoked by every /ticket-* command and by milestone-sync. Does not gate the user directly — its caller does.*

This is the shared execution layer referenced by the `/ticket-*` prompts. It contains NO user gates — the calling prompt owns all gates.

The ticket-engine is the shared procedure that the `/ticket-*` commands and the `milestone-sync` skill follow when they need to read or mutate project artifacts. It is a **shared reference doc**: it is followed inline by Copilot using its file and terminal tools, executing the procedures with its own tools (Read, Edit, Write, Bash). There is no runtime binary.
<!-- sync:end -->
...
- **How**: referenced by the calling /ticket-* prompt. The caller's prompt names which engine operation it needs (e.g. "use the engine to claim IV-042"). Copilot follows the matching § Operations entry.
- **Returns**: every operation returns a **structured result** the caller paraphrases to the user — never raw tool output. On success: `{ ok: true, artifact: {...}, steps_taken: [...] }`. On failure: `{ ok: false, where: "<step>", completed: [...], failed: "<reason>", recovery: "<manual fix>" }`. The shapes are illustrative — Copilot composes them in prose.
...
- **Config reload per invocation.** The engine re-reads `.agents/config.yaml` every time it's invoked. Config files are tiny; the cost is negligible; staleness is impossible.
- **artifact_type parameter.** Default `ticket`. Reserved values: `ticket` (live), `adr` (reserved, not implemented). Every primitive in Part 1 accepts `artifact_type` implicitly — the value affects ID prefix, commit verb namespace, and the lookup key inside `.agents/config.yaml`. Today only `ticket` is wired.
...
**Exec-bit precondition.** If `te` itself lacks the exec bit, the shell returns `126` **before** `te` runs — it cannot report on its own missing bit. Every caller (`load_and_validate()`, `/ticket-init` step 7) MUST check `[ -x .agents/scripts/te ]` first and, if it fails, emit `"te is present but not executable at .agents/scripts/te — run 'chmod +x .agents/scripts/te'; a bundle copied without exec bits is the usual cause."` The test harness asserts that only `te` is executable, so a bad copy also fails CI.
...
- **GitHub**: issue bodies carry **no frontmatter — ever**. The body is pure prose sections. Every field has a native or derived home:
  ...
  - `depends_on` → **native issue dependencies** (blocked-by relationships): `gh issue create/edit --blocked-by <n>` / `--add-blocked-by` / `--remove-blocked-by`; read back over REST via `gh api repos/{repo}/issues/{n}/dependencies/blocked_by`, gated on the issue payload's `issue_dependencies_summary.total_blocked_by`.
...
**Read side (scripted).** `te projects resolve` resolves the status field's ID + option map and each `field_map` entry's field ID + option map; it degrades to a **soft warning** (never an error) when the status field is absent, the project is deleted, or the `project` scope is missing, and is a clean no-op when `projects.enabled: false` or on a filesystem backend. The dual-homed board reads (`priority`/`effort`/`risk`) are performed by `te read`/`te list` per § Field storage contract. The **write** side below stays prose (the mutation tier).

**Never use `gh project`.** Every board operation below is a REST call through `gh api`. The `gh project *` commands are GraphQL wrappers that over-fetch badly — measured on live boards, `gh project item-list` costs **104 points on a 36-item board and 708 on a 671-item board**, and `gh project field-list` costs **102**, all against the 5,000-points/hour `graphql` bucket. The REST routes below return the same data for **1 request per 100 rows** against the separate 5,000-requests/hour `core` bucket. A board read that used to consume 14% of the hourly GraphQL budget is now ~0.1% of the REST one.

**The board route prefix** is `users/<owner>/projectsV2/<number>` for a user-owned board and `orgs/<owner>/projectsV2/<number>` for an org-owned one. **Detect the owner kind — do not trust `projects.owner_type`.** `gh api users/<owner> -q .type` returns `User` or `Organization` for either kind (org names resolve on the `users/` route). `owner_type` is unvalidated config that drifts; `gh project` resolved the owner itself so a wrong value was harmless, but over REST it points at a route that 404s and the board reads back **empty with no error** — every dual-homed field silently falling through to its label. Use `owner_type` only if the probe fails.

**ID resolution (session-cached, resolved once per engine invocation).** One request: `gh api <base>/fields`. Each entry is `{id, name, data_type, options:[{id, name:{raw}}]}` — note `id` is a **numeric** field ID (not a `PVTSSF_…` node ID) and option IDs are short hex strings.

- **Status field + options:** the `single_select` field whose name == `projects.status_field`; cache its ID and a `{ option name → option ID }` map. If no such field exists, project sync is a no-op for this run (soft warning, see below).
- **Dual-home fields:** from the same response, resolve each of `projects.field_map`'s entries (`priority` → `Priority`, `effort` → `Effort`, `risk` → `Risk` by default) to its field ID and option map. A missing board field drops that field to its label home for this run (soft warning).
- No separate project-ID lookup is needed: REST addresses items by owner + number + item ID, so the `PVT_…` project node ID the GraphQL path required has no REST equivalent to fetch.

**Add an issue to the project (on creation).** POST the issue's **numeric REST id** (`gh api repos/<repo>/issues/<n> -q .id`), not its number or URL:

```
gh api --method POST users|orgs/<owner>/projectsV2/<number>/items \
  --input - <<< '{"type":"Issue","id":<issue-numeric-id>}'
```

The response's `.id` is the item ID. If it does not surface one, recover it from `gh api <base>/items?per_page=100 --paginate` and match on `.content.html_url`.

**Set the Status (on transition / creation).** Resolve the option: `status_map[<role>]` → option name → option ID from the cached map. Writes take the **option ID**, even though reads return the option *name*:

```
gh api --method PATCH users|orgs/<owner>/projectsV2/<number>/items/<item-id> \
  --input - <<< '{"fields":[{"id":<status-field-id>,"value":"<option-id>"}]}'
```
...
- **No cycles.** A depth-first walk from `<id>` over the `depends_on` edges (ledger on filesystem; on github one paginated `gh api repos/{repo}/issues?state=all` for the `number<TAB>total_blocked_by` roll-up, closed issues included, then one `…/dependencies/blocked_by` request per issue that reports blockers) plus the proposed edges; a node recurring on the current path is reported as the full chain (e.g. `IV-007 → IV-012 → IV-007`). Never follows `related`. If an issue reports blockers but the dependencies route returns none, the engine hard-fails pointed rather than proceeding on a silently incomplete graph. Failure: `depends_on cycle: <chain>`.
...
- **Input**: none.
- **Reads**: `.agents/config.yaml` via § Config discovery.
- **Procedure**: verify `te` is executable (`[ -x .agents/scripts/te ]`; on failure emit the exec-bit message from § Config and hard-fail), then run `te config validate` — discover → parse → validate per § Config — and, on the filesystem backend, `te ledger validate` per § Ledger. Two subcommands, one logical call.
...
- **Procedure (github)**: one `gh api repos/{repo}/issues/{n}` rendered with `gh --template` (Go templates → flat lines, no jq/JSON-in-awk), plus one `…/dependencies/blocked_by` request. Numbers in these templates always go through `{{printf "%.0f" …}}` — the template engine decodes JSON numbers to float64 and renders large ones in scientific notation (a board field id came back as `3.78309328e+08`). `stage` reverse-maps the issue's status label to the config stage **key** (a **closed** issue → the terminal stage key, ignoring any stale label); `type` from the native issue type via `type_map` else the `type:` label; `priority`/`effort`/`risk` board-first-then-label (`projects.enabled`); `milestone` native title else `milestone:` label; `depends_on` from `blockedBy`; `related` from the body's `Related:` line; `claimed_by` from the assignee; `claimed_at` from the most recent `assigned` timeline event. GH has no frontmatter, so `drift` is never emitted.
...
- **Procedure (github)**: one paginated `gh api repos/{repo}/issues?state=all&per_page=100` supplies every summary. REST returns pull requests on that route, so PRs are filtered out. `blockedBy` has no bulk REST form: the payload carries `issue_dependencies_summary`, and only issues whose **`total_blocked_by`** is non-zero pay one extra `…/issues/{n}/dependencies/blocked_by` request. Read `total_blocked_by`, never `blocked_by` — the latter counts only *open* blockers, and a dep is satisfied precisely when its issue is CLOSED, so `blocked_by` would hide every satisfied edge and make `--depends-satisfied` wrongly pass. With `projects.enabled: true`, the board adds one `…/projectsV2/{n}/fields` request plus one `…/items` request per 100 items, joined by `.content.html_url`, supplying the board-homed `priority`/`effort`/`risk` (labels are not written while the board is live); projects disabled → label read only, no board calls. `claimed_at` comes from **one repo-wide `…/issues/events?per_page=100` sweep**, reduced to the latest `assigned` timestamp per issue — never one timeline request per claimed row, which measured 32 requests for a single list on a 36-issue board (the sweep was 7 pages). `te read` keeps the per-issue timeline: for one issue that is one request, and a repo-wide sweep would be absurd.
