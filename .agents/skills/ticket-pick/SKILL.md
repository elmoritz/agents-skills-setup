...
- `code-challenger` and `code-simplifier` → the two **advisory** agents the loop also runs every round. They are always on — not part of `review.agents`, not configurable away — and their output informs the evaluation without ever blocking it.
