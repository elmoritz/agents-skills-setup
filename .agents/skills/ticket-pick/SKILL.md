...
- `review.agents` → the **blocking** checkers the implementation loop runs each round (default `[code-reviewer, test-adequacy-reviewer]` when absent; a listed agent whose file is missing is skipped with a warning).
- `challenger`, and `code-challenger` and `code-simplifier` → the fixed **advisory** agents run at the Step 3 plan gate and every loop round, respectively. They are always on — not configurable away.
- `review.plan_advisors` → extra advisory agents run alongside `challenger` at the Step 3 plan gate (default empty; a listed agent whose file is missing is skipped with a warning).
- `review.advisors` → extra advisory agents run alongside `code-challenger`/`code-simplifier` every loop round (default empty; same missing-file handling).
...
Run these agents one after the other — each is read-only, none edits. If your assistant supports custom subagents, dispatch each by name (it is registered for you under `.codex/agents/`, `.gemini/agents/`, `.github/agents/`, or — on Antigravity — `.agents/agents/` itself); otherwise read the agent file and follow it inline. Same instructions either way:

- **Blocking checkers** — every agent in `review.agents` (default: `code-reviewer` (`.agents/agents/code-reviewer.md`) and `test-adequacy-reviewer` (`.agents/agents/test-adequacy-reviewer.md`), plus any a project registers). Their findings can block the loop.
- **Advisory challengers** — `code-challenger` (`.agents/agents/code-challenger.md`) and `code-simplifier` (`.agents/agents/code-simplifier.md`), always run, every round, plus every agent in `review.advisors` if configured. `code-challenger` attacks the route the code actually took; `code-simplifier` proposes behavior-preserving trims. Their output — and any configured advisor's — is advisory: it informs the evaluation but never blocks on its own.
