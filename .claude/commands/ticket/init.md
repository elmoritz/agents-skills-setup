...
1. **Detect existing agents.** Scan `.claude/agents/` for agent files other than the four shipped review agents (`challenger`, `code-reviewer`, `code-simplifier`, `test-adequacy-reviewer`). If any exist, list them and ask via `AskUserQuestion` (multiSelect):
