...
Invoke the `ticket-engine` skill at the start to load and validate config. Then:

- Resolve the `inbox` role. **If null, the command is unavailable** — print `"This project has no inbox stage configured. /ticket:refine is unavailable. Edit .claude/config.yaml to add a stage with `roles: [inbox]` if you want to enable refinement."` and stop.
...
> All gates below are asked via the `AskUserQuestion` tool. Free-text follow-ups (fold target ID, wontfix reasoning) remain inline asks.
...
Read the chosen inbox entry's body via `read_artifact(id)` and show it to the user. Then ask via `AskUserQuestion`:
...
Resume the `/ticket:new` workflow from step 2 (current implementation analysis) onward, using the inbox body as the input. Run all remaining gates (analysis, research if applicable per the type, body sections, frontmatter). This inherits `/ticket:new`'s research-agent dispatch: at steps 2 and 4, registered `research.agents` whose `consult` hints match are dispatched in parallel and their distilled findings folded in, instead of reading those sources inline. It also inherits the fixed `nfr-analyst` pass at step 2 — an inbox entry captured before the analyst existed gets its non-functional requirements derived here, and the promoted ticket carries the `## Non-functional requirements` section like any other. The "Save as inbox" option remains available at each gate since this is an inbox-enabled project; choosing it updates the existing inbox entry in place rather than creating a new one (call `update_frontmatter` with the extended body, committed as `commits.capture_update`).
